(function(){function t(n){var t=!n,e=t?{id:0,name:"",ip:"",port:9100,type:2,quantity:1,mode:0,format:80,state:1}:JSON.parse(JSON.stringify(n)),o=[{name:"前台",value:1},{name:"后厨",value:2}],s=[{name:"一菜一打",value:0},{name:"一份一打",value:1},{name:"一单一打",value:2}],h=[{name:"58mm",value:58},{name:"80mm",value:80}],c=[{name:"正常",value:1},{name:"停用",value:2}],f;$.view({title:t?"新增打印机":"编辑打印机",footDisplay:"block",template:i,load:function(){f=new Vue({el:"#edit-printer",data:{entity:e,types:o,modes:s,formats:h,states:c}});u.call(this,f)},submit:function(){var i=f.entity,u;if(!i.name){$.alert("请输入打印机名称");return}if(!i.ip){$.alert("请输入IP地址");return}if(!jdCat.utilMethods.ipValid(i.ip)){$.alert("IP地址输入错误");return}if(!i.port){$.alert("请输入端口号");return}if(!i.quantity){$.alert("请输入打印数量");return}return u=t?"/tang/addPrinter":"/tang/updatePrinter",axios.post(u,i).then(function(u){if($.alert(t?"新增成功":"修改成功","success"),t){r.items.push(u.data);return}n.name=i.name;n.ip=i.ip;n.port=i.port;n.type=i.type;n.mode=i.mode;n.format=i.format;n.quantity=i.quantity;n.state=i.state}).catch(function(n){$.alert(n)}),!0}})}function u(n){this.on("hidden.bs.modal",function(){n&&(n.$destroy(),n=null)})}var i=`
